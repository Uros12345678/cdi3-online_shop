...
class MY_Model extends CI_Model {
    ...
    public function result($select = 'id', $where = [], $start = 0, $limit = 0, $order_by = 'id', $order_type = 'description') {
        if($limit == 0) $limit = $this->config->item('per_page');
        return $this->db->select($select)->where($where)->limit($limit, $start)->order_by($order_by, $order_type)->get($this->table)->result();
    }
